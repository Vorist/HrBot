...
from config import OPENAI_API_KEY, BAD_DIALOGS_PATH, BAD_INDEX_PATH

# --- 🔑 Ініціалізація OpenAI-клієнта ---
client = OpenAI(api_key=OPENAI_API_KEY)

# --- 📥 Завантаження bad-фрагментів ---
def load_bad_chunks():
    if not os.path.exists(BAD_DIALOGS_PATH):
        print(f"❌ Файл не знайдено: {BAD_DIALOGS_PATH}")
        return []
...
def embed_texts(texts):
    try:
        print(f"📡 Генеруємо embedding-и для {len(texts)} bad-фрагментів...")
...
def build_bad_index():
    texts = load_bad_chunks()
    if not texts:
        print("⚠️ Немає валідних bad-фрагментів.")
        return
    ...
    print(f"✅ Збережено індекс з {len(embeddings)} bad-фрагментів → {BAD_INDEX_PATH}")
